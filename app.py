...
def validar_valor_ocr_soc(numero, valor):
    ...
    if numero == "13.2":
        ...
        return m.group(0) if m else ""
    if numero in {"13.3", "13.4"}:
        return valor if texto_ocr_soc_legivel(valor) else ""
    if numero == "13.5":
        return valor if texto_ocr_soc_legivel(valor, minimo_caracteres=5) else ""
    if numero == "13.6":
...
def ocr_soc_celulas_ppp(imagens):
    ...
    celulas_13_14 = [
        ...
        ("13.2", "CNPJ", 1, (0.397, 0.280, 0.550, 0.304), 7),
        ("13.3", "Setor", 1, (0.397, 0.300, 0.925, 0.316), 7),
        ("13.4", "Cargo", 1, (0.397, 0.316, 0.925, 0.336), 7),
        ("13.5", "Função", 1, (0.397, 0.329, 0.925, 0.345), 7),
        ("13.6", "CBO", 1, (0.390, 0.335, 0.500, 0.350), 7),
        ("13.7", "Código GFIP/eSocial", 1, (0.390, 0.348, 0.500, 0.364), 7),
    ...
    for numero, nome, linha, box, psm in celulas_13_14:
        ...
        if numero in {"13.2", "13.3", "13.4", "13.5", "13.6", "13.7"}:
            bruto = ocr_celula_simples(
                crop_relativo(img, box),
                psm=psm,
                whitelist=whitelist,
            )
            if adicionar_ocr_soc_validado(textos, rejeitados, numero, nome, linha, bruto):
                continue
        boxes = [box]
        if numero == "13.6":
            boxes.extend([
                (0.390, 0.337, 0.480, 0.350),
                (0.395, 0.335, 0.520, 0.351),
            ])
        elif numero == "13.7":
            boxes.extend([
                (0.390, 0.350, 0.480, 0.364),
                (0.395, 0.350, 0.500, 0.366),
            ])
    ...
    celulas_159 = [
        ("15.9 [01]", "Medidas coletivas/administrativas antes do EPI", 1, (0.835, 0.746, 0.890, 0.766), 6),
        ("15.9 [02]", "Funcionamento e uso ininterrupto do EPI", 1, (0.835, 0.770, 0.890, 0.782), 6),
        ("15.9 [03]", "Prazo de validade/CA", 1, (0.835, 0.790, 0.890, 0.802), 6),
        ("15.9 [04]", "Periodicidade de troca", 1, (0.835, 0.807, 0.890, 0.819), 6),
        ("15.9 [05]", "Higienização", 1, (0.835, 0.824, 0.890, 0.836), 6),
    ]
    for numero, nome, linha, box, psm in celulas_159:
        bruto = ocr_celula_simples(crop_relativo(img, box), psm=psm)
